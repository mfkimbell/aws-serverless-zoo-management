...
def lambda_handler(event, context):
    ...
    try:
        response = client.start_execution(
            ...
            input=json.dumps(step_function_input, cls=DateTimeEncoder),
        )
        logger.info("Start execution response: %s", json.dumps(response))
        return {"statusCode": 200, "body": json.dumps(response, cls=DateTimeEncoder)}
